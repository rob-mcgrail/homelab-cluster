...
# the bot actually appended anything new — only notify the user when
# fresh recs landed, not on every cron tick.
before=$(wc -l < "$RECS_FILE" 2>/dev/null | tr -d ' ' || echo 0)

ts=$(date -u +%Y%m%dT%H%M%SZ)
# deepseek-v4-pro — recommendations lean on taste and judgment, so the
# stronger model earns its keep here.
cd "$REPO_ROOT" && "$PI" --provider openrouter --model deepseek/deepseek-v4-pro --thinking high -a -p "$(cat "$PROMPT")" \
    > "$LOGDIR/${ts}.md" 2>&1
